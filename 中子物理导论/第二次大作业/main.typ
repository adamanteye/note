...
#grid(columns:3,column-gutter: 1em, [
  ...
  ],figure(image("9.jpg",height: 70%), caption: "Super-K剖面"),figure(image("10.jpg", height: 80%), caption: "Super-K广角摄影"))
== 成果
...
#grid(columns:2,column-gutter: 1em, [
  ...
  测量角度即可得到带电粒子的速度
  $ cos theta=1/(n beta) $
  水在20摄氏度时$n=1.33$,切伦科夫辐射的阈值速度为$0.75c$,对于电子$v_p=0.75c$,有$E=m_0c^2/sqrt(1-beta^2)=qty("0.77","MeV")$
],[#figure(image("8.svg",height: 40%), caption: "切伦科夫辐射")#figure(image("11.jpg",height: 40%), caption: [$nu_e$事例])])
== 为什么需要中子探测
Super-K缺乏反中微子探测的手段.具体而言
$ overline(nu_e)+p->e^++n $
...
#grid(columns:2,column-gutter: 1em, [
  ...
  / 氯化钆$gamma$射线重建(方法一):
    - 利用氯化钆的中子俘获反应产生的高能$gamma$射线
    - 优点:顶点,时间,能量重建效率高
  / 强制触发重建(方法二):
    - 通过引入新的强制触发逻辑实现
    - 缺点:大多数被记录的事例因为能量太低无法重建
],figure(image("12.jpg",height: 80%), caption: [实验设计]))
== 氯化钆$gamma$射线重建
使用了2.4L 0.2% $isotope("Gd")isotope("Cl")_3$水溶液,中子在其中被俘获,产生8MeV的$gamma$射线
#grid(columns: 2,figure(image("14.jpg"), caption: [$Delta T$ between prompt and delayed triggers]),figure(image("17.jpg", width: 100%), caption: [顶点重建])
)

...
== 切伦科夫辐射
#grid(columns:2,column-gutter: 2em, [
  Super-K是轻水切伦科夫探测器,收集中微子发生反应后的次级粒子的切伦科夫辐射光子(Cherenkov Radiation)

  对于以$v_p$运动的粒子,切伦科夫辐射的条件是
  $ c/n<v_p<c $
  $ cos theta=1/(n beta) $
  水在20摄氏度时$n=1.33$,切伦科夫辐射的阈值速度为$0.75c$
],figure(image("8.svg",height: 50%), caption: "切伦科夫辐射"))
== 为什么需要中子探测
...
这里产生的康普顿散射电子能量接近切伦科夫阈值($qty("0.774","MeV")$),而传统的切伦科夫探测器的触发阈值在$qty("5","MeV")$左右,因此这些中子俘获不会被记录,Super-K从而不能标记反中微子和质子的俘获 @watanabe_first_2009

如果能解决这一问题,会对超新星中微子的研究产生很大帮助

...
$]
#def([三维$delta$函数])[假设已定义一维Dirac函数$delta(x)$,那么$ delta(va(r)) & =delta(x)delta(y)delta(z) \
               & = 1 / (r^2sin theta)delta(r)delta(theta)delta(phi.alt) \
               & =1 / rho delta(rho)delta(phi.alt)delta(z) $]
== 傅里叶变换
$
...
#thm("单位算符")[完备的情况下有$ I & =sum_n ketbra(k_n) \
...
#proof[将$ket(alpha)=Delta A ket(psi)$与$ket(beta)=Delta B ket(psi)$带入@schwarz-ineq 可得$ expval((Delta A)^2) expval((Delta B)^2)>=|expval(Delta A Delta B)|^2 $由于$ Delta A Delta B=1 / 2 [Delta A,Delta B]+1 / 2 {Delta A,Delta B}\ =1 / 2 [A,B]+1 / 2 {Delta A,Delta B} $使用@hermite-re
  以及@anti-hermite-im$ expval((Delta A)^2) expval((Delta B)^2)>= & \
                      1 / 4 expval([A,B])^2 & +1 / 4 expval({Delta A,Delta B})^2 \
                                         >= & 1 / 4 expval([A,B])^2 $]
...
人们一般取$c$为实数且$c>0$,对$a^dagger$也如此操作,最后有$ a ket(n) & =sqrt(n)ket(n-1) \
a^dagger
...
#thm(
  "Feynman-Hellmann",
)[若系统哈密顿量含有某参数$lambda$,$E_n$为哈密顿量的本征值,相应归一化本征态(束缚态)为$ket(psi_n)$,有$ pdv(
    E_n,
    lambda
...
记$j_l (k a)$的根依次为$xi_(n_r l)$,那么能量本征值为$ E_(n_r l) & =hbar^2 / (2mu a^2) xi_(n_r l) \
      n_r & =0,1,2,dots $
径向本征函数为$ R_(n_r l) (r) & =C_(n_r l)j_l (k_(n_r l) r) \
...
径向本征函数为$ R_(n_r l) (r)= & a^(l+3\/2)sqrt((2^(l+2-n_r)(2l+2n_r+1)!!) / (sqrt(pi)n_r !((2l+1)!!)^2)) \
...
能量本征值为$ E_n & =-(mu e^4) / (2 hbar^2) 1 / n^2=-e^2 / (2a)1 / n^2 \
  n & =n_r+l+1 \
n_r & =0,1,2,dots $
径向本征函数为$ R_(n l) (r)=2 / (a^(3\/2)n^2(2l+1)!) & sqrt(((n+1)!) / (n-l-1)!) \
...
=== 电流分布和磁矩
统计意义上,电子的电流密度(绕z轴的环电流密度)为$ va(j) & =(i e hbar) / (2mu) (psi^* grad psi-psi grad psi^*) \
      & =-(e hbar m) / mu 1 / (r sin theta) abs(psi_(n l m))^2 vu(e_phi.alt) $
总的磁矩为$ va(M)= & -(e hbar m) / (2mu c) vu(z) integral abs(psi_(n l m))^2 dd(tau) \
     = & -(e hbar m) / (2mu c)vu(z) $
...
北大曹庆宏老师的#link("https://faculty.pku.edu.cn/caoqinghong/zh_CN/zdylm/11906/list/index.htm")[讲义草稿]非常值得阅读学习.

...
总活度为:
$
  A(t)=N_10 lambda_1 (e^(-lambda_1 t) + & \
         lambda_2 / (lambda_2-lambda_1) & (e^(-lambda_1 t)-e^(-lambda_2 t)))
$
=== 多次连续衰变规律
对于从数量为$N_10$的核素开始的多次连续衰变,衰变链中第$n$个核素的数量为
$
        N_1(t) & =N_10 e^(-lambda_1 t) \
  N_(n!=1) (t) & =N_10 product_(j=1)^n c_(n,j) e^(-lambda_j t) \
    c_(n!=1,i) & =(product_(j=1)^(n-1)lambda_j) / (product_(j=1,j!=i)^n lambda_j-lambda_i)
$
=== 长期平衡
...
/ 衰变能: $
    E_0 (beta^+)=Delta(Z, A) & -Delta(Z-1, A) \
                             & -2m_e c^2
  $
=== 禁戒条件
=== 轨道电子俘获(EC)
...
#thm("电子结合能公式")[$
  B_K (Z) & approx "Ry" (Z-1)^2 \
  B_L (Z) & approx 1 / 4 "Ry" (Z-5)^2 \
  B_M (Z) & approx 1 / 9 "Ry" (Z-13)^2
$]
== $gamma$跃迁
...
#def("反应能Q")[$
  Q & =(T_b+T_B)-(T_a+T_A) \
    ...
    & approx (Delta_a+Delta_A)-(Delta_b+Delta_B) \
    & = (B_b+B_B)-(B_a+B_A)
$]
- $Q>0$: 放能反应
- $Q<0$: 吸能反应
